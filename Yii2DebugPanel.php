...
{
    /**
     * @var null|bool подсветка кода. По умолчанию Yii2Debug::$highlightCode
     */
    ...
    {
        return Yii::app()->createUrl($this->getOwner()->moduleId . '/default/view', [
            'tag'   => $this->getTag(),
            'panel' => $this->getId(),
        ]);
    }
    ...
    {
        return $this->render(\dirname(__FILE__) . '/views/panels/_detail.php', [
            'caption' => $caption,
            'values'  => $values,
        ]);
    }
    ...
    public function renderTabs($items)
    {
        ...
        return $this->render(\dirname(__FILE__) . '/views/panels/_tabs.php', [
            'id'    => 'tabs' . ($counter++),
            'items' => $items,
        ]);
    }
    ...
    private function initLogRoute()
    {
        ...
        Yii::app()->log->setRoutes([$routeName => $this->_logRoute]);
